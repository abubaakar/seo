...
elseif($toolUid == 'PR05') {
    ...
    if ($pointOut == 'output') {
        ...
        if (filter_var($my_url, FILTER_VALIDATE_URL) === false) {
            ...
            $arr_meta = getMyMeta($my_url,$lang['ATOZAS19'],$lang['ATOZAS20'],$lang['ATOZAS21']);
    ...
    if ($pointOut == 'output') {
        ...
        $url = clean_with_www($url); $url = Trim("http://$url");
            if (!filter_var($url, FILTER_VALIDATE_URL) === false) {
                ...
                $dmozRes[] = dmozCheck($host,$lang['ATOZ111'],$lang['ATOZ112']);
            }
        }
        }
    }
...
elseif($toolUid == 'PR22') {
    ...
    if ($pointOut == 'output') {
        ...
        if ($checkDyn == '0') {
        $error = $lang['ATOZ36']; //'URL entered does not seem to be a dynamic URL';        
        } else {
...
elseif($toolUid == 'PR23') {
    ...
    if ($pointOut == 'output') {
        ...
        if (!isset($error)) {
            ...
            $outData =  checkRedirect($url,$lang['ATOZ179'],$lang['ATOZ180']);
            }
        }
    }
...
elseif($toolUid == 'PR24') {
    ...
    if($moz_access_id == null || $moz_access_id== '') $error = $lang['ATOZ209'];
    
    if($moz_secret_key == null || $moz_secret_key== '') $error = $lang['ATOZ210'];
...
elseif($toolUid == 'PR33') {
    ...
    if ($pointOut == 'output') {
        ...
        if (!isset($error)) {
            ...
            $host = $my_url['host'];
            $myHost = ucfirst(str_replace('www.','',$host));
            $file = getSiteSnap($host,$item_purchase_code, $baseLink);
            $tokenKey = randomPassword();
            $_SESSION['getWebSnap'] = $tokenKey;
            $myImage = createLink("ajax&getWebSnap&site=$host&token=$tokenKey", true, true);
            }
      }
      }
}
...
elseif($toolUid == 'PR40') {
    ...
    if ($pointOut == 'output') {
        ...
        if (!isset($error)) {
            ...
            $regUserInput = $my_url;
            $outData = checkPageSpeed($my_url,$lang['ATOZ97']);
            $timeTaken = $outData[0];
            $allLinks = $outData[1];
            $cssLinks = $outData[2];
            $imgLinks = $outData[3];
    ...
    if ($pointOut == 'output') {
        ...
        if (!isset($error)) {
            ...
            if (filter_var($my_url, FILTER_VALIDATE_URL) === false) {
                ...
                $brokenLinks = getBrokenLinks($my_url,$lang['ATOZ200']);
...
elseif($toolUid == 'PR47') {
    ...
    if ($pointOut == 'output') {
        ...
        if (!isset($error)) {
            ...
            $regUserInput = $my_url;
            $outData = spiderView($my_url,$lang['ATOZ327']);
...
elseif($toolUid == 'PR48') {
    ...
    if ($pointOut == 'output') {
        ...
        if (!isset($error)) {
            ...
            $count = 0;
            $outArr = getSuggestQueries($inData,$lang['ATOZ97']);
        }
    }
}
